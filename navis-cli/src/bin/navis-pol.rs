//! CLI driver for polarized single-inclusive hadron production,
...
use navis_pol::pdf_ff::PdfFf;

#[derive(Parser)]
#[command(about = "Polarized single-inclusive hadron production cross sections")]
struct Args {
...
fn main() -> anyhow::Result<()> {
    ...
    meta.insert("y_label".into(), "dsig/dpT (pol)".into());
    meta.insert(
        "y_label_tex".into(),
    ...
    meta.insert("convolution_type_hadron_1".into(), "PolPDF".into());
    meta.insert("convolution_type_hadron_2".into(), "PolPDF".into());
    ...
    meta.insert("sihp-pp::polarized_pdf".into(), card.polarized_pdf.clone());
    meta.insert(
        "sihp-pp::fragmentation_pdf".into(),
        card.fragmentation_pdf.clone(),
    );
    meta.insert("sihp-pp::irep_pol".into(), card.irep_pol.to_string());
